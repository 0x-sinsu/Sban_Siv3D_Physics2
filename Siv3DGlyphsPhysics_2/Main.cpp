		...
		if (it_window_width != settings.end())
		{
			...
			if (is_numeric(width_str))
			{
				try
				{
					...
					{
						System::MessageBoxOK(U"window_width "
											 U"の値は1以上の正の数で指定してください。",
				...
				{
					System::MessageBoxOK(U"window_width "
										 U"の値が無効です。数値を指定してください。",
										 MessageBoxStyle::Error);
					return;
				}
				catch (const std::out_of_range&)
				{
					System::MessageBoxOK(U"window_width の値が範囲外です。", MessageBoxStyle::Error);
					return;
				}
			}
		}
		...
		if (it_window_height != settings.end())
		{
			...
			if (is_numeric(height_str))
			{
				try
				{
					...
					{
						System::MessageBoxOK(U"window_height "
											 U"の値は1以上の正の数で指定してください。",
											 MessageBoxStyle::Error);
						return;
					}
				...
				{
					System::MessageBoxOK(U"window_height の値が範囲外です。", MessageBoxStyle::Error);
					return;
				}
			}
		}
